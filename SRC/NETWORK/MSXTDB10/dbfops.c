/* MSX trivial dropbox file operations manager 1.0
   By Konamiman 1/2011
...
const char* operationsUrl = "api.dropbox.com";
...
void GenerateHttpRequest(char* verb, char* resourceRequested, char* paramsBeforeO, char* paramsAfterO, char* destination);
...
void DoHttpGetTransaction(char* resourceName, char* paramsBeforeOauth, char* paramsAfterOauth);
...
void CreateSignatureBaseString(char* verb, char* resourceRequested, char* paramsBeforeO, char* paramsAfterO, char* destination, char* nonce, char* timestamp)
{
    ...
    pointer = CopyAndUpdatePointer(pointer, "&http%3A%2F%2F");
    pointer = CopyAndUpdatePointer(pointer, dropboxUrl);
...
void GenerateHttpRequest(char* verb, char* resourceRequested, char* paramsBeforeO, char* paramsAfterO, char* destination)
{
    char *timestamp, *signatureBaseString, *signatureKey, *unencodedSignature, *base64Signature, *percentEncodedSignature, *requestParameters, *tempPointer;
    int signatureKeyLength, baseStringLength, encodedSignatureLength;
    date currentDate;
    
    char* workArea = malloc(2048);
...
void ProcessRemoteDirectoryData()
{
    ...
    if(dirFormatType == DIR_FORMAT_RAW) {
        ...
        GetCurrentFileData(fileInfo, itemName, itemContent);
    
        if(fileInfo->isDir) {
            ShowDirContentsInformation(fileInfo, itemName, itemContent);
        } else {
...
void ShowDirContentsInformation(t_RemoteFileInfo* fileInfo, char* itemNameBuffer, char* itemContentBuffer)
{
    ...
    while(GetCurrentFileData(fileInfo, itemNameBuffer, itemContentBuffer)) {
...
void DoHttpGetTransaction(char* resourceName, char* paramsBeforeOauth, char* paramsAfterOauth)
{
    ...
    GenerateHttpRequest("GET", resourceName, paramsBeforeOauth, paramsAfterOauth, httpRequest);
...
void DoFileOperation(char* operationName, FileopType type)
{
    ...
    char* paramsBeforeO = null;
    char* paramsAfterO = null;
    ...
    if(remotePath2 == null) {
        paramsAfterO = malloc(2048);
        sprintf(paramsAfterO, "path=%s&root=dropbox", escapedPath1);
    } else {
        ReplaceChars(remotePath2, '*', ' ');
        escapedPath2 = malloc(512);
        PercentEncode(remotePath2, escapedPath2, 0);
        paramsBeforeO = malloc(2048);
        sprintf(paramsBeforeO, "from_path=%s", escapedPath1);
        paramsAfterO = malloc(2048);
        sprintf(paramsAfterO, "root=dropbox&to_path=%s", escapedPath2);
    }

    DoHttpGetTransaction(resourceName, paramsBeforeO, paramsAfterO);
