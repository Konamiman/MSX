/* MSX trivial dropbox file transfer manager 1.0
   By Konamiman 1/2011
...
const char* contentUrl = "api-content.dropbox.com";
...
void GenerateHttpRequest(bool isDownload, char* resourceRequested, char* paramsBeforeO, char* paramsAfterO, char* destination);
...
void GenerateHttpRequest(bool isDownload, char* resourceRequested, char* paramsBeforeO, char* paramsAfterO, char* destination)
{
    char* verb = isDownload ? "GET" : "POST";
    char *timestamp, *signatureBaseString, *signatureKey, *unencodedSignature, *base64Signature, *percentEncodedSignature, *requestParameters, *tempPointer, *contentLengthString=null;
    int signatureKeyLength, baseStringLength, encodedSignatureLength;
    date currentDate;
    char* workArea = malloc(4096);
    
    //printf("*** paramsBeforeO: %s\r\n", paramsBeforeO);
    ...
    if(isDownload) {
        sprintf(destination,
            "GET %s%soauth_consumer_key=%s&oauth_token=%s&oauth_nonce=%s&oauth_signature_method=HMAC-SHA1&oauth_timestamp=%s&oauth_signature=%s&oauth_version=1.0 HTTP/1.1\r\n"
            "Host: %s\r\n"
//            "Connection: close\r\n"
            "%s\r\n"
            "\r\n",
        ...
        sprintf(destination,
            "POST %s?%s&oauth_consumer_key=%s&oauth_token=%s&oauth_nonce=%s&oauth_signature_method=HMAC-SHA1&oauth_timestamp=%s&oauth_signature=%s&oauth_version=1.0 HTTP/1.1\r\n"
            "Host: %s\r\n"
...
void ComposeUserAgentString()
{
    ...
    pointer = malloc(128);
    sprintf(userAgent, "User-Agent: MSX trivial dropbox/1.0 (MSX-DOS %s; TCP/IP UNAPI; %s)", GetDosVersionString(pointer), GetUnapiImplementationName(pointer + 16));
    mfree(pointer);
...
void DoGetHttpTransaction(char* resourceName, char* fullLocalName)
{
    ...
    GenerateHttpRequest(true, resourceName, null, null, httpRequest);
...
void GetOneFile(char* remotePath, char* localName, bool localNameIsDir)
{
    ...
    if(fileExists && skipExistingFiles) {
        ...
        strcpy(resourceName, "/0/files/dropbox/");
...
void DoPostHttpTransaction(t_FileInfoBlock* fib, char* remoteName)
{
    ...
    CombinePaths("/0/files/dropbox/", directoryName, resourceName);
    
    originalFileNameLength = strlen(fileNamePart);
    GenerateHttpRequest(false, resourceName, paramsBeforeOauth, null, httpRequest);
    SendTcpData(httpRequest, strlen(httpRequest));
    mfree(resourceName);
    mfree(directoryName);
    print(strOk);
